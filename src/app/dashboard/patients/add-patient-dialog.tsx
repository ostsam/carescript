...
const RELATIONS = ["Spouse", "Son", "Daughter", "Sibling", "Friend", "Other"] as const;
...
export function AddPatientDialog({ children }: { children: React.ReactNode }) {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        setPatientInfo({
          ...
          lovedOneFirstName: formData.get("lovedOneFirstName")?.toString() ?? "",
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg" showCloseButton={step === "details"}>
        {step === "details" ? (
          <DetailsStep
            formRef={formRef}
...
function VoiceStep({
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      setCloneError("Microphone access denied. Please allow microphone access and try again.");
    }
  }, []);
  ...
  async function handleCloneVoice() {
    ...
    try {
      ...
      formData.append(
        "description",
        `Voice clone of ${lovedOneName} (${patientInfo.lovedOneRelation}) for patient ${patientInfo.patientFirstName} ${patientInfo.patientLastName}`,
      );
  ...
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>
          Record a sample of <span className="font-medium text-foreground">{lovedOneName}</span>&apos;s
          voice. At least 30 seconds of clear speech works best.
          This step is optional — you can add a voice later.
        </DialogDescription>
      </DialogHeader>

      <div className="grid gap-4">
        <div className="rounded-xl border bg-muted/30 p-4" style={{ minHeight: 112 }}>
          {!recording && !audioBlob && (
            <div className="flex h-20 flex-col items-center justify-center gap-2">
              <div className="flex size-10 items-center justify-center rounded-full bg-primary/10">
                <HugeiconsIcon icon={Mic01Icon} size={20} className="text-primary" />
              </div>
          ...
          {!recording && audioBlob && (
            <div className="flex h-20 flex-col items-center justify-center gap-1">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-medium">{formatTime(elapsed)} recorded</span>
              </div>
      ...
      <DialogFooter>
        <Button type="button" variant="ghost" onClick={onDone} disabled={cloning}>
          Skip for now
        </Button>
        <Button
          type="button"
