...
export const AudioScrubber = ({
	...
	const derivedProgress = duration > 0 ? currentTime / duration : 0;
	const progress = isDragging ? localProgress : derivedProgress;
...
export const LiveMicrophoneWaveform = ({
	...
	const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
		...
		e.preventDefault();
		setScrubMax(historyRef.current.length);
		setIsDragging(true);
	...
	const canScrub = !active && scrubMax > 0;
	...
	return (
		<div
			...
			aria-valuenow={canScrub ? Math.abs(dragOffset) : undefined}
			aria-valuemin={canScrub ? 0 : undefined}
			aria-valuemax={canScrub ? scrubMax : undefined}
